...
The paper surveys and compares relevant methods: ESM3, gLM2, protein attention/contact-map work, Structure Language Models, BioEmu, Distributional Graphormer, DynaFold, latent diffusion on graph embeddings, AlphaFlow, P2DFlow, MDGen, Boltzmann Generators, small-molecule conformer generators, AlphaFold3, RoseTTAFold All-Atom, and nucleic-acid language models. It also develops a theoretical universal BioSELFIES tokenization layer for representing all biomolecular data through a robust SELFIES-style grammar, then refines that idea into a more efficient hybrid multiresolution tokenizer that keeps all-atom and side-chain dynamics where they matter. The accompanying implementation now includes compact BioSELFIES scale graphification, UniProt feature and biomolecular-affinity graphification, PPI/affinity contact records, all-atom contact templates with bond edge tokens, two GFlowNet modes, UMA coordinate/internal-coordinate heads, and explicit curation and integrity checks. The paper therefore distinguishes the executable scaffold from the longer-term scientific claims, and gives mathematical definitions, training objectives, temperature-conditioning mechanisms, implementation phases, datasets, curation methods, evaluation protocols, failure modes, and a practical research plan for testing the hypothesis.
\end{abstract}
...
The accompanying repository implements a first executable version of the strict symbolic-input path. It should be understood as an implementation scaffold and data pipeline, not yet as evidence that the full scientific hypothesis is true. The implemented \(\bioselfies\) layer is a total decoder for a supported bracket grammar:
\[
...
The current coordinate path is also implemented as an oracle-feedback path rather than a supervised structure path. Source-side \texttt{UMA\_COORD\_QUERY:*} and all-atom Cartesian query records are derived from symbolic molecule or sequence records; for proteins the generator starts from sequence-derived heavy-atom slots, beginning with backbone atoms \(N,C_\alpha,C,O\) and continuing into side-chain heavy atoms under the configured atom budget. DNA, RNA, and ligand SELFIES records similarly create symbolic atom slots and covalent bond edge tokens. These slots are \emph{candidate output positions}; they are not copied from PDB, AlphaFoldDB, SDF, mmCIF, or MD files. A gated coordinate head reads continuous positions from the same hidden graph-of-thought embeddings that drive graph-token decoding. UMA/FairChem then scores the generated candidate coordinates at the graph's Kelvin temperature and returns detached forces. The training surrogate
\[
...
The implemented contact-map path is also graph-token based. Contact maps and attention maps are not stored as residue-only matrices. Structure-dynamics rows can insert an unfolded all-atom template derived from protein, DNA, RNA, or SELFIES strings: atoms are source nodes and covalent bonds are \texttt{molecular\_bond} edge tokens. TokenGT attention can therefore attend over residue/base/SELFIES tokens, atom tokens, and bond edge tokens in one graph, subject to the context budget. The model may also consume non-structural contact priors: ESM-style predicted contacts, gLM2/OMG-style categorical-Jacobian contacts, and affinity-weighted PPI or complex-interface records. These are hypotheses or scalar/annotation-derived priors, not coordinate-derived ground truth contact maps. In particular, not every contact record has affinity data: affinity weighting is available for biomolecular-complex and PPI rows carrying \(K_d,K_i,\mathrm{IC}_{50},k_{\mathrm{on}},k_{\mathrm{off}}\), or \(\Delta G\)-style values, while ESM and categorical-Jacobian contacts remain unweighted coupling priors unless joined to an affinity row.

The implemented GFlowNet path is split into two modes. The SFT GFlowNet, \texttt{gflownet.mode: sft}, learns diverse symbolic graph completions over ordinary target tokens: function descriptions, UniProt annotations, assay metadata, molecule strings, text, code, math, and tool/proof rows. The structure-dynamics GFlowNet, \texttt{gflownet.mode: structure\_dynamics}, narrows the candidate vocabulary to BioSELFIES, all-atom Cartesian records, internal-coordinate actions, contact patches, adaptive patches, temperature records, token-motion records, and UMA/oracle records. This split was introduced because the broad symbolic SFT target distribution and the structure-dynamics construction distribution have different entropy, reward, and candidate-vocabulary requirements.

Generated structure-dynamics outputs are represented as model-generated all-atom Cartesian frames plus metadata. The repository includes export scaffolding for multi-frame PDB files and trajectory-style files suitable for downstream MD tooling. The PDB export can include secondary-structure records or viewer hints when secondary-structure assignments are available or inferred, but those annotations are output/rendering metadata; they are not supervised structure labels in the strict training path.

\subsection{Datasets and curation methods in the implementation}

The data pipeline is local-first and explicitly split by information provenance. The strict training path permits symbolic sequences, molecular strings, non-structural annotations, scalar assay or affinity values, and online oracle feedback. It excludes supervised coordinate labels, structure-token labels, contact labels derived from structures, energy/force labels from datasets, conformer libraries, and MD trajectories. Evaluation and future ablation sources may include structures or trajectories, but those sources must be kept out of the strict policy-training mix.

\begin{longtable}{p{0.22\linewidth}p{0.34\linewidth}p{0.34\linewidth}}
\caption{Implemented data lanes and their strict-training status.}\\
\toprule
Lane & Sources and row types & Role in the current scaffold\\
\midrule
\endfirsthead
\toprule
Lane & Sources and row types & Role in the current scaffold\\
\midrule
\endhead
Broad bio sequence scale & UniRef50 protein sequences, PubChem10M SELFIES, UniProt function text, Rfam, RNAcentral 8192, and DNA coding-region rows & Compact BioSELFIES/sequence graph rows capped at 8192 BioSELFIES/source characters in the current runner. The runner targets roughly 3M protein rows, at least 3M molecule SELFIES rows, at least 3M RNA rows, and source-limited DNA rows where the public split is smaller. These rows train symbolic biological grammar and modality coverage, not all-atom coordinates.\\
Reviewed UniProt features & Local UniProt feature exports with accessions, names, organism/taxon, GO, keywords, EC numbers, domains, PTMs, variants, cofactors, catalytic activity, subcellular location, subunit text, binding sites, active sites, metal-binding sites, DNA-binding sites, and other sequence features & Symbolic protein/function/binding-site graph rows. The current local export contains 574,627 data rows. Binding and active-site annotations are sequence annotations; they are not coordinate labels.\\
Biomolecular-complex affinity & Local rows for protein-protein, protein-RNA, protein-DNA, protein-ligand, ligand-nucleic-acid, antibody-antigen, receptor-ligand, or arbitrary component complexes carrying \(K_d,K_i,\mathrm{IC}_{50},k_{\mathrm{on}},k_{\mathrm{off}}\), \(\Delta G\), temperature, pH, buffer, and assay metadata & Scalar affinity supervision and affinity-weighted interface priors. The current local graphified source contains 1,836,729 data rows. These rows emit complex, component, affinity, \texttt{AFFINITY\_CONTACT:*}, \texttt{PPI\_CONTACT:*}, and \texttt{CONTACT\_PATCH:affinity\_weighted\_interface} records when the row supports them.\\
Original full selected public mix & GraphWalks, GraphInstruct-style graph reasoning, math/tool reasoning, Hebrew, molecule reasoning, UniGenX-style science rows, and other selected public graph rows & General UGM graph-reasoning and language/science substrate. The completed local mix contains 7,328,008 graph rows and can be included with the UniProt/affinity corpus for the combined training run.\\
Contact-prior sources & ESM-style predicted protein contacts, OMG/gLM2 mixed CDS/intergenic rows, categorical-Jacobian contact records, and affinity-derived PPI/interface records & Contact hypotheses for graph construction. ESM/Jacobian records are priors; only rows joined to affinity data become affinity-weighted. No PDB-derived contact labels are permitted in the strict path.\\
Static structure/contact subset & A curated high-signal subset carrying all-atom Cartesian action labels, all-atom contact-template atoms, covalent bond edge tokens, contact patches, and relevant sequence/affinity context & Default size is 25,000 rows in the current bio-scale runner. These are candidate/action records and unfolded sequence-derived templates, not supervised native structures.\\
Structure-dynamics subset & Rows with static/contact records plus internal-coordinate, UMA/oracle, token-motion, adaptive-patch, contact-patch, temperature, or affinity-contact targets & Default size is 2,500 rows in the current runner. This subset feeds the structure-dynamics GFlowNet and UMA coordinate/internal-coordinate stages.\\
\bottomrule
\end{longtable}

Curation proceeds in five explicit steps. First, source-specific preparation scripts normalize TSV, CSV, JSON, JSONL, FASTA, or parquet rows into a common graph JSONL schema with typed nodes, typed edges, target tokens, metadata, optional coordinate-query slots, and no forbidden structure labels. Second, the curation pass deduplicates by exact raw-row or configured row hashes, applies optional quality filters, and splits by entity rather than by row when possible. Entity keys include protein sequence keys, molecule/scaffold keys, document keys, and fallback row hashes. Third, the fast-copy mode streams JSONL lines directly to split files rather than holding the full corpus in memory; resumable temporary split files and a resume-state file allow interrupted curation to continue. Fourth, repair and integrity tools check for JSONL concatenation errors, split-size mismatches, and unreadable rows. Fifth, policy checks verify that strict runs do not contain PDB/mmCIF/SDF/trajectory coordinate labels, supervised energy or force labels, or structure-derived contact labels.

The largest completed combined run used the reviewed UniProt feature graph file, the biomolecular-complex affinity graph file, and the original full selected public graph corpus. Its curation input was 9,739,364 rows before exact duplicate removal; the resulting split was approximately 8.7M train rows, 0.52M validation rows, and 0.50M test rows. One 250M UGM SFT epoch over that combined corpus completed, followed by a 3,000-step SFT GFlowNet stage and a 3,000-step structure-dynamics GFlowNet stage. A newer bio-scale run uses compact BioSELFIES graphification for the broad million-row modality sources and then reuses the same all-atom contact/affinity/UMA training stack.
