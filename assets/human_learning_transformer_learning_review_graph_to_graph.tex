...
\title{\vspace{-0.5in}\textbf{Human Learning at Cellular Resolution, Agentic Transformer Learning, and Graph-to-Graph Multimodal Molecular Generation}\\[0.3em]
\large A comparative review and engineering program with TokenGT graph-to-graph transduction, persistent topology, tropical geometry, looped reasoning dynamics, and all-atom molecular structure generation}
...
The central architectural convention used in the remainder of the manuscript is stronger than saying that graphs are convenient inputs. The model class is treated as a family of \emph{graph-to-graph transducers}. A standard text sequence, a theorem-proof trace, a SELFIES string, a protein residue list, an RNA secondary structure, an all-atom conformer, and a multi-frame PDB file are all represented first as typed graph-record objects. A sequence is allowed only as an interface serialization of such an object.
...
\]
over output graphs. The flattened text string \(s\), SELFIES string \(m\), proof script \(\pi\), or PDB trajectory \(P\) is obtained only after decoding by a deterministic or stochastic renderer
\[
  s=\mathrm{flat}_{\rm text}(Y_G),\qquad
  m=\mathrm{flat}_{\rm selfies}(Y_G),\qquad
  \pi=\mathrm{flat}_{\rm proof}(Y_G),\qquad
  P=\mathrm{flat}_{\rm PDB}(Y_G).
\]
The renderer may impose canonical ordering, line formatting, atom numbering, or surface grammar, but it is not the mathematical object on which the learning theory is based.
\end{definition}

This convention changes the interpretation of earlier transformer terminology. ``Next-token prediction'' becomes a special case in which the output graph is a path graph and the renderer reads the path left-to-right. A proof assistant transcript is a dependency graph whose printed Lean text is a serialization. A molecular trajectory is a sequence of frame subgraphs sharing atom identities and bond records; its printed PDB file is a lossy but useful rendering. Thus the default map is graph-to-graph, while graph-to-sequence appears only as \(\mathrm{flat}\circ f_G\) after graph decoding.
...
Let \(\mathrm{flat}:\mathfrak G_Y\to\Sigma^{\le T}\) be a deterministic serializer, for example a left-to-right text renderer, Lean-script printer, SELFIES printer, or multi-\texttt{MODEL} PDB writer. If \(f_G:\mathfrak G_X\to\mathfrak G_Y\) is approximable in the graph-to-graph sense and \(\mathrm{flat}\) is continuous on the chosen finite-precision graph-record topology or stable on a positive-margin subset, then \(\mathrm{flat}\circ f_G\) is approximable as a sequence-valued output. Thus graph-to-sequence modeling is a consequence of graph-to-graph modeling, not the primitive assumption.
\end{corollary}

This theorem is not a claim about efficient learnability. It says that expressivity is not the main obstruction for bounded graph-language, proof, and molecular transductions. Practical obstructions include graph construction quality, token budget, optimization, data, invariance generalization, and the cost of attention over node-edge-hyperedge-frame tokens. The important conceptual change is that text, code, proof scripts, SELFIES strings, and PDB files are surface serializations of output graphs.
...
\section{Integrated extension: random-order multimodal TokenGT for molecular structure and dynamics}
\label{sec:integrated_ugm_extension}

The following integrated extension corrects two limitations at once: treating the molecular architecture as a separate standalone proposal, and treating generation as primarily graph-to-sequence. It is written as a continuation of the same theory: language, proof states, molecular sequences, chemical graphs, persistent-topological summaries, tropical selection regions, and physical trajectory records are all graph-structured information processed by one TokenGT-only graph-to-graph model.

\section{Motivation and thesis}

The preceding sections argued that language, memory, and reasoning can be treated as graph-structured information moving through latent dynamical systems. This paper makes that thesis concrete in a molecular setting. The proposed model receives arbitrary mixtures of natural language, protein sequence, SELFIES, DNA, and RNA. It converts the mixture into an input typed graph; performs reasoning, structure construction, and tool use as graph completion; and decodes an output typed graph. Text answers, SELFIES renderings, Lean scripts, coordinate tables, and multi-frame PDB files are obtained only by flattening the output graph. Natural-language requests, residues, atoms, base pairs, bonds, distance bins, motif memberships, thought nodes, tool calls, conformer frames, and PDB rows are all records in one graph grammar.

The architectural rule is intentionally restrictive and is now embedded into the longer comparative paper rather than treated as a detached proposal. A protein residue is not sent to a protein module. A SELFIES string is not sent to a chemistry module. Coordinates are not generated by an equivariant coordinate head. There is no separate AlphaFold-style pairformer and no learned VQ structure alphabet as the final target. The transformer is a TokenGT-style graph-to-graph model over node, edge, hyperedge, frame, and attribute records. The allowed changes are: vocabulary augmentation; graph-record serialization as an implementation detail rather than a mathematical output type; random-order autoregressive graph decoding; conditioning tokens for temperature, modality, units, and task; and auxiliary losses that judge whether the decoded output graph is chemically and physically plausible.
...
\node[gbox,below=of self,minimum width=3.2cm,minimum height=0.9cm] (nuc) {DNA/RNA\\bases + structural motifs};
\node[box,right=1.0cm of prot,minimum width=4.5cm,minimum height=3.6cm] (graph) {Unified typed graph\\[0.25em]\begin{tabular}{c}vertices: words, residues, atoms, bases, thoughts\\edges: bonds, dependencies, contacts, base pairs\\records: distances, coordinates, frames, temperature\end{tabular}};
\node[pbox,right=1.0cm of graph,minimum width=4.1cm,minimum height=2.0cm] (model) {TokenGT-only\\graph-to-graph backbone\\random-order graph decoding};
\node[obox,right=0.9cm of model,minimum width=4.0cm,minimum height=1.4cm] (outg) {Output graph\\text nodes + atom/bond/coord/frame records};
\node[obox,above right=0.0cm and 0.9cm of outg,minimum width=3.8cm] (text) {Flatten to text\\function, proof, protocol};
\node[obox,below right=0.0cm and 0.9cm of outg,minimum width=3.8cm] (pdb) {Flatten to multi-\texttt{MODEL} PDB\\all-atom frames};
\node[box,below=0.8cm of model,minimum width=4.2cm] (uma) {UMA-style MLIP oracle\\energy, forces, relaxation, reward};
...
\end{tikzpicture}}
\caption{High-level graph-to-graph architecture. All modalities are converted into one input graph-record language and decoded into one output graph-record language. Text and PDB files are flattenings of the output graph; geometry is learned by random-order graph decoding and loss-level physical supervision rather than by a specialist structure module.}
\label{fig:overview}
\end{figure}

The research hypothesis has four parts. First, graph-token modeling is a natural common interface for language, molecules, proteins, nucleic acids, and agentic tool traces. Second, random-order autoregressive decoding is a better inductive bias for output graph records than left-to-right decoding because proofs, molecules, complexes, and conformational trajectories have no privileged surface sequence order. Third, persistent topology and tropical geometry provide useful diagnostics and regularizers for latent graph reasoning: topology tracks stable multi-scale shape, while tropical geometry tracks max-like selection and piecewise-linear decision structure. Fourth, an MLIP oracle can provide physical feedback without changing the architecture. The oracle supplies energies, forces, relaxation directions, and temperature-conditioned rewards; the model still emits graph tokens.
...
Protein sequence and protein generation systems are also relevant. ProGen2 scales autoregressive protein sequence modeling to billions of parameters \cite{NijkampProGen2_2023}. ESM3 jointly reasons over protein sequence, structure, and function with discrete tracks \cite{HayesESM3_2025}. ProstT5 and Foldseek's 3Di alphabet show that structure-derived alphabets can be useful for protein representation and search \cite{HeinzingerProstT5_2023,VanKempenFoldseek2024}. This paper treats such alphabets as motif-mining tools, not as final structure targets. The model may use 3Di-like fragments to create structure-derived motif tokens, but it must ultimately emit atom identities, bonds, distance records, coordinates, and PDB frames.

SELFIES is used because every valid SELFIES string maps to a valid molecular graph under its semantic constraints \cite{KrennSELFIES2020}. Group SELFIES extends this idea with fragment-level tokens \cite{ChengGroupSELFIES2023}. Again, the string is not enough. A graph-valid molecule may have many conformers, so the target is an ensemble of coordinate graphs conditioned on temperature and context.

The proposal therefore sits between three traditions: graph-token transformers such as TokenGT \cite{KimTokenGT2022}; multimodal molecular structure predictors such as AlphaFold 3, Boltz, Chai, and RoseTTAFold-All-Atom; and generative molecular models trained by diffusion, flow matching, or GFlowNets \cite{GatDFM2024,LouSEDD2024,BengioGFlowFoundations2023,VolokhovaConfGFN2024}. Its novelty is the refusal to add a molecular architecture. It asks whether sufficiently rich graph records, random-order decoding, and physical oracle feedback can make a pure transformer graph learner generate useful all-atom structures and conformational trajectories.
...
\]
where $V$ is a finite set of vertex tokens, $E\subseteq V\times V\times \mathcal R_E$ is a finite set of typed edge records, $\tau_V$ and $\tau_E$ are type maps, $a_V$ and $a_E$ are attribute maps, $R$ is a finite set of non-edge records such as coordinates, distances, PDB rows, temperature tags, and tool observations, and $\Gamma$ stores global conditioning variables: modality mask, task, units, temperature, chain identifiers, decoding policy, and requested output format.
\end{definition}

The same schema covers every modality. In language, vertices are words, subwords, phrases, proof states, claims, equations, thoughts, tool calls, and observations. Edges are syntax, semantic roles, discourse relations, citations, derivations, graph-of-thought links, and tree-of-thought branches. In proteins, vertices include residues, atom slots, motifs, secondary-structure segments, domains, and chains. Edges include peptide bonds, residue adjacency, contact bins, hydrogen-bond candidates, disulfide bonds, motif membership, and all-atom bonds. In SELFIES, vertices include SELFIES symbols, atoms, functional-group motifs, stereochemical markers, and ring/branch records. In DNA/RNA, vertices include bases, sugar/phosphate atom slots, base-pair motifs, stacking edges, loop motifs, helices, pseudoknot markers, and sequence motifs.
...
\]
where each record $r_i$ is a typed token tuple. Examples include \texttt{NODE(residue, chain=A, index=12, aa=W)}, \texttt{EDGE(peptide, v12, v13)}, \texttt{DIST(atom34, atom91, bin=7.5-8.0A)}, \texttt{COORD(atom34, frame=3, xbin=..., ybin=..., zbin=...)}, and \texttt{TEXT(role=function, span=...)}. A serializer maps this record set to an ordered list for batching, printing, or file export, but the learned object remains graph-to-graph.
\end{definition}

\subsection{Proteins with residue and structure-derived motif vocabularies}

Protein input vocabulary begins with the twenty standard amino acids. It is extended by sequence motifs, domains, and structure-derived motifs. Sequence motifs are mined from resources such as InterPro, PROSITE, Pfam-through-InterPro, and CATH-Gene3D annotations. Structure-derived motifs may be mined by clustering local residue neighborhoods, torsion patterns, contact patches, secondary-structure fragments, or Foldseek/3Di-like strings. The target remains all-atom geometry, not a structure-code sequence.

For all-atom decoding, residues are expanded into virtual atom slots. The template includes backbone slots $N,C_\alpha,C,O$ plus a superset of side-chain slots large enough to cover the largest standard amino acids. The decoded residue identity activates a mask. For glycine, side-chain slots are inactive. For tryptophan, the aromatic side-chain slots are active. Nonstandard residues can be represented either by a template extension or by falling back to atom-level SELFIES-like graph records.
...
\subsection{SELFIES small molecules}

SELFIES tokens are converted to molecular graphs containing atoms, bonds, charges, aromaticity, ring closure, branches, chirality, and optional group motifs. The output graph includes atom records, bond records, distance records, conformer-frame coordinate records, energy records, and PDB/HETATM serialization records. Motif tokens may include common functional groups, ring systems, fragments from Group SELFIES, pharmacophores, and substructures mined from ChEMBL, ZINC, PubChem, or GEOM. Fragment tokens are compression devices; they do not replace atom-level supervision.

\subsection{DNA and RNA}

DNA and RNA inputs use base tokens $A,C,G,T,U$, ambiguity codes when needed, backbone atom slots, sugar pucker tokens, base-pair tokens, stacking tokens, loop motifs, helix motifs, and RNA-family motif tokens. Rfam covariance models and RNAcentral sequence clusters provide sequence and family supervision. NAKB, PDB/NDB-derived structures, and RNA 3D Hub representative sets provide coordinate, base-pair, and motif supervision. Modified bases are represented by template extensions or atom-level graph records.

\subsection{Mixed complexes and cross-modal prompts}

The model is designed for mixtures: a natural-language task may reference a protein sequence, a ligand SELFIES string, an RNA sequence, and an intended temperature. Cross-modal edges attach task constraints to molecular subgraphs. Examples include \texttt{ASKS\_ABOUT(prompt, chainA)}, \texttt{BINDS(ligand, pocket)}, \texttt{TARGET\_TEMP(prompt, 300K)}, and \texttt{FUNCTION\_OF(text, protein)}. Mixed complexes are not a special architecture; they are larger graphs.
...
\[
  X_G \longmapsto Y_G,
\]
not as \(X_G\longmapsto y_1,\ldots,y_T\). The input graph \(X_G\) contains prompt nodes, sequence nodes, motif nodes, partial structure nodes, retrieved context, tool observations, and temperature tokens. The output graph \(Y_G\) contains one or more of the following: explanatory text nodes connected by discourse and citation edges; proof or code nodes connected by dependency edges; atom, bond, distance, torsion, coordinate, energy, and force records; trajectory-frame subgraphs; and serializer records such as PDB row templates. A final renderer may flatten \(Y_G\) to prose, Lean, Python, SELFIES, SDF/mmCIF, or PDB.
...
Protein/RNA/DNA structure & Atom-slot nodes, residue/base nodes, covalent and noncovalent edges, distance/coordinate/frame records & PDB/mmCIF trajectory \\
Small-molecule conformers & Atom nodes, bond-type edges, stereo/chirality records, conformer frames, energy/force records & SELFIES, SDF, PDB/HETATM block \\
...
Protein residues & 20 standard amino acids, chain markers, residue indices, secondary-structure tags, torsion-bin records & Protein sequence and residue-level structure. \\
Protein motifs & PROSITE/InterPro motifs, CATH domains, clustered local contact motifs, structure-derived fragments & Compression, inductive bias, and function/structure annotation. \\
...
DNA/RNA & bases, ambiguity symbols, base-pair tokens, stacking tokens, loop/helix motifs, sugar pucker, modified bases & Nucleic-acid sequences, secondary structure, and 3D records. \\
Geometry & atom-slot tokens, bond records, distance bins, torsion bins, coordinate bins, frame tokens, PDB row tokens & Direct all-atom structure output. \\
Dynamics/oracle & temperature, energy, force, relaxation, validity, reward bins & UMA-style supervision. \\
Attention & incidence, priority, confidence & Routing supervision. \\
\bottomrule
...
Bond type is a first-class edge token. A molecular edge without bond type is insufficient for all-atom generation because valence, aromaticity, stereochemistry, protonation, conjugation, force-field assignment, and PDB/SDF/mmCIF export all depend on edge identity. \UGM{} therefore represents a covalent edge as a typed record
\[
...
\end{aligned}
\]
SELFIES-derived molecules require single, double, triple, aromatic, ring-closure, branch, stereochemical, formal-charge, and valence records. Proteins require peptide, disulfide, hydrogen-bond, salt-bridge, metal-coordinate, and ligand-covalent records. DNA and RNA require phosphodiester, glycosidic, Watson-Crick, wobble, Hoogsteen, sugar-edge, Leontis-Westhof orientation, and stacking records. Bond-type tokens are used both as input features and as decoded outputs; validators parse them before coordinate losses or UMA scoring are trusted.
...
Motifs must be mined only from training splits. For proteins, cluster sequence windows, domain annotations, secondary-structure patterns, local contact neighborhoods, and 3Di-like structural strings. For small molecules, mine frequent subgraphs and functional groups. For RNA, mine covariance-model families, base-pair loops, and 3D motifs. For language, mine parse motifs, proof motifs, and GoT/ToT motifs. Motif vocabularies are then frozen before validation and test data are processed.
...
def mine_motifs(train_examples, modality, min_support, max_size):
    ...
    for x in train_examples:
        ...
        for subgraph in enumerate_connected_subgraphs(g, max_size=max_size):
            code = canonicalize(subgraph, ignore_coordinates=False)
...
The requested attention-weighting paradigm is implemented as data and loss, not as a new attention operator. For a graph with attention matrices $A^{(\ell,h)}$, define a sparse coupling prior $P$ over token pairs. $P_{ve}$ is high when vertex $v$ is incident to edge token $e$; $P_{ev}$ is high when edge $e$ should return information to vertex $v$; $P_{dd}$ is high for distance records sharing atoms; $P_{tt}$ is high for adjacent trajectory frames; and $P_{tool}$ is high for tool-call/result pairs. Add
\[
  \mathcal L_{\mathrm{attn}} = \sum_{\ell,h}\lambda_{\ell,h}\,\mathrm{KL}\big(P\,\|\,A^{(\ell,h)}+\epsilon\big).
\]
This encourages vertex-to-edge and edge-to-vertex routing but leaves the TokenGT block unchanged. The same weights also determine decoding-order proposals: high-priority bond and valence records are decoded before fine coordinate bins; force-critical atoms receive more refinement steps.

\section{Random-order autoregressive graph decoding}

Left-to-right decoding is appropriate for many rendered text strings, but neither an output proof graph nor a molecular graph is a privileged string. Atom order in a PDB file is a convention; bond existence, coordinates, and contacts are exchangeable up to graph isomorphism and chain conventions. Random-order decoding trains the model to complete arbitrary missing records of an output graph conditioned on an input graph.
...
\end{tikzpicture}}
\caption{Random-order graph-to-graph autoregression. The next output record may be an atom, bond, distance, coordinate bin, text node, tool result, or trajectory-frame record, always conditioned on the input graph and the partial output graph.}
\label{fig:random}
\end{figure}
...
def training_step(example):
    records = graphify(example)                 # set of typed graph records
    order = sample_order(records, policy_mix=[uniform, coarse_to_fine, energy_aware])
    prefix, targets = [], []
    loss = 0
    for r in order:
        ...
        prefix.append(r)
    loss += geometry_losses(prefix) + chemistry_losses(prefix)
    loss += oracle_losses(prefix) + attention_coupling_loss(model.attn)
    return loss
\end{lstlisting}

\section{All-atom structure as graph records}

The output structure is a decoded graph plus frames. For frame $f$, atom $i$ has identity $z_i$, optional residue membership, bond relations, and coordinates $x_i^{(f)}\in\R^3$. The model emits atom, bond, distance, coordinate, frame, and metadata records as an output graph; distance records provide redundant constraints and PDB rows are only a later serialization.

\begin{definition}[All-atom graph frame]
An all-atom frame is
\[
  Y_f=(V_A,E_B,Z,X_f,D_f,C_f),
\]
where $V_A$ is the atom set, $E_B$ is the typed bond set, $Z$ gives element and atom labels, $X_f\in\R^{|V_A|\times 3}$ gives coordinates, $D_f$ gives selected pairwise distance records, and $C_f$ gives constraints such as valence, chirality, chain membership, and residue templates.
\end{definition}

\begin{proposition}[Distance-coordinate consistency]
If a complete squared distance matrix $D$ is Euclidean of rank at most three, then classical multidimensional scaling reconstructs coordinates in $\R^3$ up to rigid motion and reflection.
\end{proposition}
\begin{proof}
Let $J=I-n^{-1}\mathbf 1\mathbf 1^T$ and $B=-\frac12JD^{(2)}J$. If $D$ is Euclidean in $\R^3$, $B=XX^T$ for centered coordinates $X$ and has rank at most three. Eigen-decomposition of $B$ gives coordinates up to an orthogonal transform.\end{proof}

The model should emit both sparse distances and coordinates. Distances stabilize geometry and allow local consistency checks; coordinates provide the PDB output. A repair pass can project inconsistent distances and coordinates onto a valid molecular geometry using constrained optimization, classical geometry, or an external relaxation tool.

\subsection{Virtual side-chain and nucleotide templates}

\begin{figure}[H]
\centering
\resizebox{0.85\linewidth}{!}{%
\begin{tikzpicture}[font=\small,node distance=6mm]
\node[gbox,minimum width=3.4cm] (res) {Residue token\\\texttt{AA=W}};
\node[box,right=of res,minimum width=4.1cm] (slots) {Universal residue template\\backbone + max side-chain slots};
\node[pbox,right=of slots,minimum width=3.6cm] (mask) {Identity mask\\activate Trp slots};
\node[obox,right=of mask,minimum width=3.6cm] (coords) {Coordinate records\\active atoms only};
\draw[edge] (res)--(slots); \draw[edge] (slots)--(mask); \draw[edge] (mask)--(coords);
\node[gbox,below=of slots,minimum width=4.3cm] (nuc) {Nucleotide template\\phosphate + sugar + base slots};
\draw[dedge] (nuc)--(mask);
\end{tikzpicture}}
\caption{Template-slot decoding. The model can allocate a uniform residue or nucleotide template and then trim inactive atoms according to decoded identity.}
\end{figure}

\subsection{PDB trajectory serialization}

A generated trajectory is serialized as multi-frame PDB. Each frame is a \texttt{MODEL}/\texttt{ENDMDL} block with \texttt{ATOM} and \texttt{HETATM} rows. Bond information can be emitted as \texttt{CONECT} records when appropriate; distances and metadata can be emitted in sidecar JSON or REMARK records.

\begin{lstlisting}[caption={PDB trajectory writer.}]
def write_multimodel_pdb(frames, atoms, bonds, metadata):
    out = []
    for k, X in enumerate(frames, start=1):
        out.append(f"MODEL     {k:4d}")
        for i, atom in enumerate(atoms, start=1):
            x,y,z = X[i]
            out.append(format_atom_row(i, atom, x, y, z))
        for (i,j,typ) in bonds:
            out.append(format_conect_row(i, j, typ))
        out.append("ENDMDL")
    return "\n".join(out) + "\nEND\n"
\end{lstlisting}

\section{Temperature-conditioned conformational dynamics}

The model receives an explicit temperature token $T\in\{400,375,350,325,300\}\,\mathrm K$. The target is not one structure but a distribution over conformations. If $E(Y)$ is the oracle potential energy, the desired equilibrium density is proportional to
\[
  p_T(Y\mid X)\propto \exp\{-E(Y)/(k_B T)\}\,\mathbf 1[\mathrm{valid}(Y)].
\]
Exact sampling is unavailable; the paper proposes three approximations.

\subsection{Direct UMA-style energy-force distillation}

Given frames $Y_f$, query an oracle MLIP for energy $E_f$ and forces $F_f=-\nabla_X E(Y_f)$. Train the TokenGT model to predict energy bins, force-direction bins, force-magnitude bins, and relaxation records. The model does not receive a force head; it emits graph records such as \texttt{FORCE\_BIN(atom=17, frame=2, dir=..., mag=...)}.

\subsection{Energy and force binned motion supervision}

A more direct way to use the oracle is to discretize local motion. For each atom and frame, compute the force vector and assign it to a directional bin on the sphere and a magnitude bin. Add records that tell the model how the graph should move under local relaxation. At inference, the model can generate a frame, query or approximate force bins, emit a motion proposal, and resample. This is especially useful for the annealed ladder: at high temperature, allow wider motion bins; at low temperature, restrict to smaller refinements.

\begin{lstlisting}[caption={Constructing a UMA-labeled annealed ladder.}]
def build_ladder(seed_structure, temperatures=[400,375,350,325,300]):
    frames = []
    y = seed_structure
    for T in temperatures:
        for step in range(n_steps(T)):
            E, F = UMA(y)
            move_bins = bin_forces(F, temperature=T)
            y_prop = propose_discrete_motion(y, move_bins, T)
            E_prop, F_prop = UMA(y_prop)
            accept = metropolis(E, E_prop, T) and chemistry_valid(y_prop)
            y = y_prop if accept else y
            frames.append(record_frame(y, T, E_prop, F_prop, accept))
    return frames
\end{lstlisting}

\subsection{GFlowNet objective}

Let a trajectory $\tau$ be a sequence of graph-completion actions ending in a structure $Y$. Define reward
\[
R_T(Y)=\exp\{-\beta_T E_{\mathrm{UMA}}(Y)-\lambda V(Y)\}\,Q(Y),
\]
where $\beta_T=(k_BT)^{-1}$, $V(Y)$ penalizes valence, clash, chirality, and template violations, and $Q(Y)$ is an optional task score such as pocket compatibility or function agreement. Trajectory balance trains forward and backward policies so terminal samples are proportional to $R_T$.

\begin{theorem}[Trajectory-balance optimum]
Assume a finite acyclic construction graph, positive terminal rewards, and expressive forward/backward policies. At a global optimum of the trajectory-balance objective, the terminal distribution of the forward policy is proportional to $R_T(Y)$.
\end{theorem}
\begin{proof}
The standard trajectory-balance equations enforce equality between the total flow into a terminal object and its reward. Summing over all trajectories leading to a terminal state gives terminal flow proportional to $R_T$. Normalizing flows gives the forward terminal distribution.\end{proof}

\section{Datasets and curation}

The dataset program has five layers: language and reasoning; sequence corpora; structure corpora; conformational ensembles and energy-force data; and synthetic verified tool traces. All splits must control leakage. Protein splits should be clustered by sequence identity and temporal release. Small molecules should be split by scaffold and sometimes by time. RNA should be split by family and motif. Motif vocabularies are mined only from training data.
...
\midrule
\endhead
Language & FineWeb, Dolma, OpenWebMath, The Stack, tool logs, Lean/math corpora & Instruction, math, code, GoT traces, explanation. & Refs. \\
Protein sequence & UniProt, UniRef, UniParc, metagenomic resources when license permits & Protein sequence modeling, family clustering, function annotation, motif mining. & Refs. \\
Protein structure & RCSB PDB, wwPDB, AlphaFold DB, ProteinNet & Residue/atom graph targets, distograms, coordinate records, temporal splits. & Refs. \\
Protein motifs & InterPro, PROSITE, CATH-Gene3D, Foldseek/3Di-derived local patterns & Domain tokens, sequence motifs, local structural motif tokens. & Refs. \\
SELFIES molecules & PubChem, ChEMBL, ZINC, MoleculeNet, QM9, PubChemQC & SELFIES and graph pretraining, property labels, small-molecule coverage. & Refs. \\
Conformers and energy & GEOM, SPICE, rMD17, OMol25 & Conformer ensembles, DFT labels, energy/force supervision, oracle calibration. & Refs. \\
DNA/RNA sequence & GenBank, RefSeq, Ensembl, RNAcentral, Rfam & Nucleotide sequence pretraining, RNA families, covariance/secondary structure. & Refs. \\
DNA/RNA structure & PDB/NAKB, RNA 3D Hub, Rfam structures & Coordinates, base pairs, stacking, loops, motifs, nucleic-acid complexes. & Refs. \\
Dynamics & ATLAS, mdCATH, GEOM trajectories, UMA-generated ladders & Temperature-conditioned trajectories and multi-frame PDB targets. & Refs. \\
Oracle & UMA, OMol25 models, MACE, EquiformerV2 & Energy-force feedback, relaxation labels, baseline energy/force validation. & Refs. \\
\bottomrule
\end{longtable}}
...
def graphify_example(raw):
    ...
    if modality == "language":
        ...
        if raw.structure: g.add(atom_graph_and_distances(raw.structure))
    elif modality == "selfies":
        mol = selfies_to_mol(raw.selfies)
        g = molecular_graph(mol)
        g.add(fragment_motifs(mol, selfies_motif_vocab))
    elif modality in ["DNA", "RNA"]:
        g = nucleotide_graph(raw.sequence)
        g.add(nucleic_motifs(raw.sequence))
        if raw.structure: g.add(nucleic_structure_records(raw.structure))
...
Text output should include biochemical function descriptions, but those descriptions must be grounded. Construct pairs from UniProt function fields, Gene Ontology annotations, enzyme commission numbers, InterPro domains, PDB ligand annotations, ChEMBL target annotations, and curated literature abstracts. The curation rule is conservative: a generated function target is included only when the graph contains explicit supporting annotations. For medicine-design prompts, use target-ligand structures, assay metadata, and property labels to build explanatory tasks such as: predict binding-pocket contacts, explain strained torsions, propose conformer refinement, or describe why a ligand violates valence or stereochemical constraints. These are training tasks for reasoning and tool use, not claims of therapeutic efficacy.
...
\]
Here $\mathcal L_{\mathrm{ROA}}$ is random-order autoregressive cross entropy over output graph records conditioned on input graph records. $\mathcal L_D$ predicts distance bins; $\mathcal L_X$ predicts coordinate bins or residual quantization; $\mathcal L_B$ predicts bonds; $\mathcal L_C$ penalizes chemical invalidity; $\mathcal L_E$ and $\mathcal L_F$ distill energy and force records from oracle data; $\mathcal L_{\mathrm{GFN}}$ is trajectory-balance or flow-matching loss; $\mathcal L_{\mathrm{attn}}$ supervises vertex-edge couplings; and $\mathcal L_{\mathrm{top/trop}}$ regularizes persistent and tropical summaries.

\subsection{Persistent topology and tropical geometry}

For a point cloud of latent embeddings or predicted atomic coordinates, compute persistent summaries such as connected components, loops, and multi-scale cluster lifetimes. Use these as diagnostics and optional losses: generated conformer ensembles should preserve appropriate topological signatures of known folds or molecular graphs. Tropical geometry enters through max-plus selection. In low-temperature attention,
\[
\mathrm{softmax}(s/\tau)\rightarrow \arg\max_i s_i\quad\mathrm{as}\quad\tau\rightarrow0,
\]
and ReLU networks are piecewise affine over polyhedral regions. Tropical diagnostics track sharp region changes during graph decisions: selecting a bond, contact, motif, thought branch, or conformer basin. A tropical regularizer can penalize unstable max selections under small input perturbations unless the oracle reward strongly supports a sharp transition.

\section{Sampling algorithms}

\subsection{Random-order autoregressive sampling}

\begin{lstlisting}[caption={Annealed random-order sampler.}]
def sample_ugm(context, temperatures=[400,375,350,325,300]):
    records = initialize_records(context)
    trajectory = []
    for T in temperatures:
        records.add(global_token(f"TEMP_{T}K"))
        while not complete_for_temperature(records, T):
            scope = choose_missing_scope(records, policy="energy_aware")
            ...
            if should_query_oracle(records):
                records.add(UMA_feedback_records(records))
        trajectory.append(extract_frame(records, T))
    return write_multimodel_pdb(trajectory)
\end{lstlisting}

\subsection{Masked discrete diffusion-style sampling}

Initialize all target records as \texttt{MASK}. At each step, the model predicts a subset of masked records conditioned on the visible graph, then remasks or refines low-confidence records. Coordinate records can move from coarse bins to fine bins. The schedule can be temperature-aware: high-temperature steps denoise broad basins; low-temperature steps refine local geometry.

\subsection{Discrete flow-matching-style sampling}

Treat graph records as categorical states and train a time-indexed model to map a base masked distribution toward the data distribution. The flow predicts replacement rates for record types. For molecular graphs, a useful schedule first creates atom and residue identity, then bonds and valence, then distances, then coordinates, then PDB serialization. Unlike standard left-to-right decoding, flow steps can update many records in parallel.

\subsection{GFlowNet sampling}

GFlowNet sampling is used when diversity matters. Instead of seeking one best structure, the model samples multiple structures with probability proportional to a reward. For conformers, the reward is Boltzmann-like under UMA energy, corrected by validity and task constraints. This is a natural fit for multi-frame PDB output: each frame can be treated as a terminal object or a waypoint in an annealed trajectory.


\section{Theoretical implications}

\begin{theorem}[Finite graph representability]
For any finite collection of language, protein, SELFIES, DNA, RNA, and all-atom molecular examples with bounded size and finite precision coordinates, there exists a finite graph-record vocabulary extension that represents every input example and every output example exactly.
\end{theorem}
\begin{proof}
Each example contains finitely many discrete labels and finitely many numerical coordinates. Quantize numerical values to the required finite precision. Add a token or tuple schema for each required label, relation, and quantization bin. The union over a finite dataset is finite. Encoding each object as node, edge, hyperedge, attribute, frame, and serializer records gives exact representation at the chosen precision.
\end{proof}

\begin{theorem}[Multimodal TokenGT graph-to-graph representability]
Fix bounded graph-record classes for the four input modalities considered here: natural language/reasoning graphs, protein graphs, SELFIES molecular graphs, and DNA/RNA graphs. Fix a bounded output class containing text graphs, proof/code graphs, all-atom molecular graphs, and multi-frame trajectory graphs. At finite coordinate precision, every deterministic target map from these input classes to the output class can be represented as a map between finite record sets; if the map is continuous in the record-feature topology, it is uniformly approximable by a TokenGT-style graph-to-graph transformer in the sense of the universality theorem above.
\end{theorem}
\begin{proof}
...
For two conformer graphs \(Y_1,Y_2\) with \(E(Y_1)<E(Y_2)\) and identical validity terms, the reward ratio \(R_T(Y_1)/R_T(Y_2)\) increases as \(T\) decreases.
\end{proposition}
\begin{proof}
The ratio is \(\exp((E(Y_2)-E(Y_1))/(k_BT))\). Since \(E(Y_2)-E(Y_1)>0\), the derivative with respect to \(1/T\) is positive.
\end{proof}

\begin{corollary}[PDB and text generation are graph-rendering problems]
If \(Y_G\) is decoded correctly as an output graph, then any deterministic canonical renderer produces a valid surface object whenever its preconditions are satisfied. In particular, a multi-frame PDB trajectory is obtained by ordering atom records, frame records, and bond records; a biochemical function paragraph is obtained by ordering claim/evidence/discourse records; and a proof script is obtained by topologically sorting proof-dependency records.
\end{corollary}
\begin{proof}
Each renderer is a function on graph records. If the required records exist and satisfy the renderer constraints, applying the renderer yields the corresponding surface string. Topological sorting is possible when dependency edges form a DAG; PDB rendering is possible when atom identifiers, residue or molecule identifiers, coordinates, and record-compatible elements are present; text rendering is possible when discourse edges and token spans define an ordered surface realization. None of these operations changes the learned graph-to-graph map.
\end{proof}

The propositions clarify why the annealed ladder is useful. At \(400\,\mathrm K\), sampling explores broader basins. At \(300\,\mathrm K\), rewards become sharper and the sampler concentrates around lower-energy conformers, provided the oracle is calibrated and the validity penalties are strong. They also clarify why the paper insists on graph-to-graph modeling. The target of a scientific agent is rarely a string alone: it is a structured object whose serialization may be prose, code, proof text, SELFIES, SDF, mmCIF, or PDB.
...
\midrule
\endhead
0 & Schema and vocabulary & Define record grammar, motifs, coordinate bins, temperature tokens, PDB writer, validators. & Training splits only. \\
...
3 & Structure supervision & Emit atom, bond, distance, coordinate, distogram, and PDB records. & PDB, AlphaFold DB, NAKB, RNA 3D Hub, GEOM, QM9. \\
4 & Oracle distillation & Predict UMA/MLIP energy bins, force bins, relaxation direction, and validity. & OMol25, SPICE, rMD17, UMA-labeled generated frames. \\
5 & Temperature ladder & Train $400\to300\,\mathrm K$ annealed trajectories and multi-frame PDB output. & ATLAS, mdCATH, GEOM, UMA-generated ladders. \\
6 & GFlowNet fine-tuning & Sample terminal structures proportional to Boltzmann-like reward. & Generated and curated conformer trajectories. \\
...
for batch in dataloader:
    ...
    loss_geom = distance_loss(pairs) + coordinate_loss(pairs) + bond_loss(pairs)
    loss_chem = valence_loss(pairs) + chirality_loss(pairs) + clash_loss(pairs)
    if has_oracle_labels(batch):
        loss_oracle = energy_loss(pairs) + force_bin_loss(pairs)
    if gflownet_stage:
        loss_gfn = trajectory_balance_loss(model, pairs, reward=UMA_reward)
    loss_attn = incidence_attention_kl(model.attention_maps, pairs)
    loss = weighted_sum(loss_roa, loss_geom, loss_chem, loss_oracle, loss_gfn, loss_attn)
...
\begin{table}[H]
\centering
\caption{Structure prediction validation template.}
\scriptsize
\setlength{\tabcolsep}{3pt}
...
\midrule
CASP-style protein domains & TM-score, lDDT, RMSD & & & \\
PDB temporal split & chain lDDT, contact F1 & & & \\
Protein-ligand complexes & ligand RMSD, pocket RMSD, contact F1 & & & \\
RNA 3D Hub split & RMSD, INF, base-pair F1 & & & \\
Small-molecule GEOM & COV/MAT, RMSD, validity & & & \\
Multi-frame conformers & ensemble coverage, energy diversity & & & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Temperature-conditioned dynamics validation template.}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{0.21\linewidth}p{0.21\linewidth}p{0.13\linewidth}p{0.13\linewidth}p{0.08\linewidth}}
\toprule
Task & Metric & 400K & 300K & Notes \\
\midrule
UMA agreement & energy MAE, force cosine & & & \\
Boltzmann ranking & Spearman energy rank & & & \\
Annealed sampling & acceptance, diversity, clash rate & & & \\
ATLAS/mdCATH ensembles & RMSF correlation, contact persistence & & & \\
Small-molecule torsions & torsion KL, coverage & & & \\
PDB validity & parser success, bond validity & & & \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
Variant & Expected advantage & Structure & Diversity & Cost \\
\midrule
Left-to-right serialization & baseline & & & \\
Uniform random-order AR & order invariance & & & \\
Coarse-to-fine random-order AR & physical curriculum & & & \\
Masked discrete diffusion & parallel refinement & & & \\
Discrete flow matching & categorical transport & & & \\
GFlowNet with UMA reward & Boltzmann-weighted diversity & & & \\
Annealed ladder & exploration then refinement & & & \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
PDB repair after validation failure & successful repair rate & & & \\
Function description from sequence/structure & factual F1 against annotations & & & \\
Tool-using conformer search & oracle-improved samples per call & & & \\
Graph-of-thought molecular design & valid diverse candidates & & & \\
...
The architecture is high-risk. Pure TokenGT may be less sample efficient for geometry than equivariant networks. If so, the value of the program may be multimodal graph reasoning and structure-aware tool use rather than state-of-the-art coordinate accuracy. PDB generation is fragile: a file can parse and still be chemically wrong. UMA-like MLIPs are not exact quantum mechanics and may fail out of distribution, especially for unusual charge states, rare elements, reactive intermediates, solvent effects, long-range electrostatics, and quantum phenomena. Temperature-conditioned samples are not automatically thermodynamic ensembles. Any application to medicine design requires experimental validation, medicinal chemistry review, toxicity assessment, pharmacokinetic modeling, synthesis feasibility, and regulatory safeguards.

\section{Conclusion}

\UGM frames language, proteins, SELFIES molecules, DNA, RNA, all-atom structures, and conformational trajectories as one graph-to-graph record modeling problem. It preserves a TokenGT-only architecture while introducing the necessary domain structure through vocabulary, random-order graph decoding, curation, losses, attention-coupling supervision, serializers, and UMA-style physical feedback. The most important empirical question is not whether this immediately defeats specialized structure predictors. It is whether a graph-token transformer can learn a unified conditional distribution over output graphs--including explanations, proof/code traces, all-atom structures, and temperature-conditioned trajectories--well enough to become a useful scientific reasoning and structure-generation agent.

\appendix
\section{Concrete prompt-target formats}

\subsection{Protein folding with temperature}
\begin{lstlisting}
INPUT GRAPH:
  NODE task: PREDICT_STRUCTURE
  NODE chainA: PROTEIN_SEQUENCE "MKWVTFIS..."
  NODE temp: TEMP_300K
  EDGE asks_about(task, chainA)
TARGET RECORDS:
  NODE residue_i; EDGE peptide_i_i+1; NODE atom_slots;
  DIST atom_i atom_j bin=...; COORD atom_i frame=1 xbin=... ybin=... zbin=...;
  TEXT function_description "...";
  PDB MODEL records.
\end{lstlisting}

\subsection{Ligand conformer ensemble from SELFIES}
\begin{lstlisting}
INPUT GRAPH:
  SELFIES tokens + functional group motif tokens + TEMP_375K
TARGET:
  atom graph, bond graph, multiple coordinate frames, UMA energy bins,
  force-bin relaxation records, multi-MODEL PDB serialization.
\end{lstlisting}
...
def make_tool_trace(example):
    ...
    draft = model_or_teacher_generate(records, task="structure_and_explain")
    checks = run_validators(draft, tools=[pdb_parser, rdkit, openmm, uma])
...
\begin{lstlisting}[caption={Structure-derived motif construction.}]
def structure_motifs(structures):
    windows = []
    for s in structures:
        contacts = contact_graph(s, cutoff=8.0)
        for center in residues(s):
            patch = radius_subgraph(contacts, center, radius=2)
            patch.add(torsion_bins(s, patch.residues))
            patch.add(secondary_structure(s, patch.residues))
            windows.append(canonical_patch_code(patch))
    return cluster_and_name(windows, split_safe=True)
\end{lstlisting}
...
\midrule
\endhead
TM-score/lDDT/RMSD & protein/RNA & Fold similarity, local geometry, and aligned coordinate error. \\
Contact F1 & protein, RNA, complex & Precision and recall for predicted contacts or base pairs. \\
COV/MAT & small molecules & Coverage and matching of conformer ensembles. \\
Energy MAE & all molecular & Mean absolute error against UMA, DFT, or force-field energy labels. \\
Force cosine & all molecular & Directional agreement between predicted force bins and oracle force vectors. \\
Valence violation & chemistry & Fraction of generated graphs with invalid valence, charge, or bond order. \\
Clash rate & structure & Fraction of atom pairs violating steric-distance thresholds. \\
PDB parse rate & output & Fraction of generated PDBs parsed by independent parsers. \\
RMSF correlation & dynamics & Agreement of residue/atom fluctuation profiles with MD ensembles. \\
