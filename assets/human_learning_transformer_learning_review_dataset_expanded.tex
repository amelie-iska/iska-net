...
The codebase now also contains an optional continuous coordinate head for a later explicitly enabled structure phase. It is disabled in the sequence-first full run. Let \(q\) be a \(\texttt{<POS>}\) query slot whose symbolic target is a coordinate record. The transformer hidden state \(h_q\) is mapped to
\[
  (\mu_q,\ell_q)=H_{\rm coord}(h_q)\in\mathbb R^3\times\mathbb R^3,
\]
where \(\mu_q\) is a normalized mean coordinate and \(\ell_q\) is a clipped log-variance vector. If \(y_q\in\mathbb R^3\) is an available coordinate target, \(m_q\in\{0,1\}^3\) is the axis mask, and \(s>0\) is a coordinate scale, the masked coordinate loss is
\[
  \mathcal L_{\rm coord}
  =
