...
\]
The renderer may impose canonical ordering, line formatting, atom numbering, trajectory container layout, or surface grammar, but it is not the mathematical object on which the learning theory is based. PDB, mmCIF, SDF, XTC, DCD, or other trajectory files are downstream renderings of generated graph states, not training targets in the strict curriculum.
\end{definition}
...
\subsection{Multi-head Tropical Attention as an optional UGM backend}

The NeurIPS 2025 Tropical Attention proposal replaces the dot-product/softmax attention kernel by an operation in tropical projective space \cite{HashemiTropicalAttention2025}. The relevant point for UGM is not that softmax attention should be abandoned everywhere. The relevant point is that many graph reasoning tasks contain an internal dynamic program: shortest paths, reachability, maximum-weight dependency structures, Viterbi paths, proof-state selection, graph-of-thought branch choice, and knapsack-like resource allocation. These tasks are naturally written with maxima, minima, and ordinary addition. Tropical Attention gives the transformer a direct max-plus computation path for such decisions.

\begin{definition}[Tropical projective representative]
Let \(x\in\R^d\). A finite tropical projective representative is obtained by a valuation-like map
\[
  \phi(x)=u-\max_r u_r\,\1,\qquad
  u=\log(1+\operatorname{ReLU}(Wx)).
\]
Thus \(\max_r \phi(x)_r=0\). Two tropical vectors \(a,b\in\R^d\) are projectively equivalent when \(b=a+c\1\) for some scalar \(c\). The quotient by this relation is the finite part of tropical projective space.
\end{definition}

\begin{definition}[Tropical Hilbert projective metric]
For \(a,b\in\R^d\), define
\[
...
\begin{lemma}[Projective invariance of \(d_H\)]
For any scalars \(c,d\in\R\),
\[
  d_H(a+c\1,b+d\1)=d_H(a,b).
\]
\end{lemma}
\begin{proof}
The coordinate difference becomes
\[
(a_r+c)-(b_r+d)=(a_r-b_r)+(c-d).
\]
Both the maximum and the minimum over \(r\) are shifted by the same scalar \(c-d\), so their difference is unchanged.
\end{proof}

\begin{definition}[Multi-head Tropical Attention]
Let \(X\in\R^{n\times d}\) be a token-hidden matrix and let \(H\) divide \(d\). For head \(h\), write \(d_h=d/H\). Tropical Attention forms tropicalized projections
\[
  Q^{(h)}= \Phi(XW_Q^{(h)}),\qquad
  K^{(h)}= \Phi(XW_K^{(h)}),\qquad
  V^{(h)}= \Phi(XW_V^{(h)}),
\]
optionally followed by head-wise max-plus linear maps
\[
  (\mathcal A\odot z)_o=\max_r\{\mathcal A_{or}+z_r\}.
\]
...
\[
  C_i^{(h)}
  =
  \bigoplus_{j=1}^n S_{ij}^{(h)}\odot v_j^{(h)}
  =
  \max_j\{S_{ij}^{(h)}+v_j^{(h)}\},
\]
...
\begin{proposition}[A tropical attention head realizes a weighted max gate]
Fix scalars \(x_j\) and weights \(w_j\) for \(j\in J\). A tropical attention head can realize
\[
...
\end{proposition}
\begin{proof}
Assign the value vector at token \(j\) to contain \(x_j+w_j\) in the output coordinate of interest. Choose query and key representatives so that the Hilbert distance from the query to keys in \(J\) is \(0\), hence their score is \(0\). For every irrelevant token \(r\notin J\), choose its key so that \(d_H(q,k_r)=\Gamma_r\) with \(\Gamma_r\) large. Its score is then \(-\Gamma_r\). The tropical context is
\[
\max\left(\max_{j\in J}(0+x_j+w_j),\max_{r\notin J}(-\Gamma_r+v_r)\right).
\]
Taking all \(\Gamma_r\) larger than the finite value range suppresses irrelevant tokens and leaves \(\max_{j\in J}(x_j+w_j)\).
\end{proof}

\begin{theorem}[Piecewise-linear tropical attention cells]
For fixed parameters and fixed masks, a Multi-head Tropical Attention block without the final Euclidean devaluation is a piecewise-linear map of its tropicalized inputs. Its cells are determined by finitely many equalities of coordinate differences and max-plus value comparisons.
\end{theorem}
\begin{proof}
...
\begin{corollary}[Dynamic-program compatibility]
Any finite max-plus dynamic-program update of the form
\[
...
\begin{proof}
The update is exactly a weighted max gate over predecessors \(u\). The preceding proposition realizes such a gate at a token. Applying the construction in parallel over \(v\) and stacking layers over \(t\) unrolls the bounded-horizon dynamic program.
\end{proof}

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=1.1cm, every node/.style={font=\small}, >=Latex]
\node[box, minimum width=2.6cm] (x) {Euclidean hidden\\states \(X\)};
\node[gbox, right=of x, minimum width=2.7cm] (val) {log-ReLU\\valuation \(\Phi\)};
\node[gbox, right=of val, minimum width=2.8cm] (qkv) {max-plus\\\(Q,K,V\)};
\node[obox, below=of qkv, minimum width=3.0cm] (hilbert) {Hilbert score\\\(S_{ij}=-d_H(q_i,k_j)\)};
\node[obox, left=of hilbert, minimum width=3.2cm] (ctx) {max-plus context\\\(\max_j(S_{ij}+v_j)\)};
\node[pbox, left=of ctx, minimum width=2.8cm] (out) {devaluation\\and output proj};
\draw[edge] (x) -- (val);
\draw[edge] (val) -- (qkv);
\draw[edge] (qkv) -- (hilbert);
\draw[edge] (hilbert) -- (ctx);
\draw[edge] (ctx) -- (out);
\draw[dedge] (out) -- ++(0,-0.8) -| node[pos=0.25, below] {residual block} (x);
\end{tikzpicture}
\caption{Multi-head Tropical Attention path used as an optional UGM backend. The attention kernel is max-plus; the surrounding residual, normalization, and feed-forward structure remains transformer-like.}
\label{fig:mhta_backend}
\end{figure}

For UGM, the practical conclusion is a config-gated backend rather than a mandatory replacement. The default model remains a standard TokenGT-style transformer. The pure optional backend sets \texttt{model.attention\_backend=tropical}, uses masked Hilbert scores in every encoder attention block, supports causal and padding masks, logs \texttt{tropical\_attention/*} metrics, and is intended first for graph-reasoning ablations. The training backend used for larger local runs is hybrid and layer-sparse: \texttt{model.attention\_backend=hybrid\_flash\_tropical} keeps FlashAttention or scaled-dot-product softmax attention in every layer, then activates MHTA only on the configured layer set \(\mathcal L_{\rm trop}\). With the default \(\mathcal L_{\rm trop}=\{L\}\), only the final encoder layer pays the explicit Hilbert-distance cost while earlier layers remain on the fast attention path. The implementation computes the exact MHTA scores in query chunks for long-context training, avoiding materialization of the full \([B,H,S,S,d_h]\) difference tensor while preserving the same max-plus/Hilbert operation. This preserves a max-plus computation channel near the decision surface without forcing every intermediate representation through the quadratic MHTA tensor.

This distinction is important because the Tropical Attention paper's empirical evidence is strongest for combinatorial reasoning and long/value/noise out-of-distribution tests; large autoregressive language and molecular graph training remain an experimental scaling question. The most useful first ablations are therefore: standard attention, standard attention with tropical logit diagnostics, full Multi-head Tropical Attention, layer-sparse hybrid FlashAttention+MHTA, layer-sparse hybrid plus topology diagnostics, and layer-sparse hybrid plus verifier/GFlowNet graph-of-thought rewards.
...
The present repository state is already nontrivial. The completed public graph corpus contains $7{,}328{,}008$ examples, approximately $1.00\times 10^9$ source graph tokens, $4.89\times 10^7$ target graph tokens, and $1.11\times 10^9$ untruncated graph-sequence tokens; the 4090-bounded public corpus contains $230{,}857$ examples. Since the first version of this paper, the implementation has also materialized a local biomed layer with $574{,}627$ reviewed UniProt feature rows and $1{,}836{,}729$ biomolecular complex-affinity rows. The combined local-biomed-plus-original curation input is $9{,}739{,}364$ rows before exact duplicate accounting, with a completed split of roughly $8.7$M train examples, $0.52$M validation examples, and $0.50$M test examples. A 250M-parameter UGM run completed one full SFT epoch over that combined corpus, followed by a $3{,}000$-step SFT GFlowNet stage and a $3{,}000$-step structure-dynamics GFlowNet stage.

The current train-ready mixture is strongest in Hebrew text, mathematical reasoning, selected formal-proof data, sequence-first molecular records, sanitized UniGenX-style metadata rows, reviewed UniProt annotations, and scalar affinity records for biomolecular complexes. It remains weakest in four places: truly large-scale balanced protein/RNA/DNA sequence-function coverage, full multimodal PPI/contact supervision with measured affinity on every row, long-horizon high-fidelity structure-dynamics validation, and explicit graph-algorithm or graph-of-thought supervision at scale. Those weaknesses matter because the proposed architecture is not a text-only language model. It is a typed graph-to-graph transducer whose most important learned skill is to transform one relational object into another.

SFM/NatureLM and UniGenX currently enter the codebase in two distinct ways. First, their reference repositories and papers provide methodology and reference vocabulary; extracted NatureLM/UniGenX tokens extend the graph vocabulary but are not counted as train/validation/test examples by themselves. Second, selected UniGenX-style public sources are used only after sequence-first sanitization: molecule rows keep SELFIES/SMILES-style strings and non-structure metadata, while coordinate and physics fields are excluded from the first training curriculum. A separate NatureLM-style public-source acquisition path now downloads and prepares official PubChem, UniProt, UniRef, Rfam, RNAcentral, RefSeq, and DNA coding-region slices into graph JSONL splits. The default preparation includes the tractable public slices and records provenance; full PubChem CID-SMILES, UniProt TrEMBL, very large UniRef clusters, and complete nucleotide archives are high-cardinality paths that must be enabled deliberately, and Materials Project remains API-gated unless an \texttt{MP\_API\_KEY} export is supplied.

The current model scale is approximately $0.5$B parameters and the near-term training budget is about $2$B consumed tokens, plus GFlowNet and UMA-style oracle training. This budget cannot literally reproduce the raw-token scale of recent looped latent-reasoning language models, which train from multi-trillion-token web corpora and report $7.7$T-token training runs \cite{ZhuOuro2025,FineWebEdu2024,DCLM2024,Dolma2024}. The proper target is therefore two-tiered. First, build a candidate source pool that is large enough and diverse enough to support future scaling. Second, for the $2$B-token run, select a high-density curriculum in which many tokens carry typed edges, verifiers, motifs, graph states, sequence annotations, or oracle-feedback records. The training objective is not raw token maximalism; it is high mutual information between a small number of tokens and the graph-to-graph transformations the model must learn.
...
The default 2026 full-run selection is capped by an operational budget of at most \(5\times 10^9\) untruncated graph-sequence tokens. The bio-scale run uses a compact BioSELFIES representation so that high-volume protein, molecule, RNA, and DNA rows do not explode into dense all-atom graph records. Broad sequence rows remain compact; all-atom Cartesian slots, all-atom contact templates, covalent bond edge tokens, PPI/contact records, affinity records, and UMA coordinate/internal-coordinate query slots are concentrated in the structure/contact subsets. The sources below were selected by expected marginal quality gain for this architecture: explicit graph-state reasoning first, then sequence/function grounding for the four scientific modalities, then verifier/tool reasoning, then a curated general-language slice. If the token guard fails after graphification, the lowest-ranked rows are reduced first; OpenAI GraphWalks remains a default source because it is the most direct available supervision for long-context graph traversal and answer-node prediction.
...
3 & PubChem10M SELFIES & High-volume molecule string coverage without structure-file inputs. & Cap at 8M rows in general; bio-scale target $\geq 3$M when disk and time allow. \\
4 & UniRef50 protein sequences & Large-scale protein sequence diversity with cluster-aware split keys and compact BioSELFIES serialization. & Bio-scale target about 3M rows. \\
5 & UniProt function text descriptions & Protein sequence-to-function grounding for function-description outputs. & Include by default; combine with reviewed UniProt features. \\
6 & Rfam & RNA family sequence and family/clan annotation. & Bio-scale target about 3M rows where source rows permit. \\
7 & RNAcentral 8192 & Broad RNA sequence diversity with type/description metadata and long sequence support. & Bio-scale target about 3M rows. \\
8 & DNA coding regions & DNA sequence, exon/intron, and translated-protein annotations. & Target about 3M rows, source-limited in the current manifest. \\
9 & Biomolecular affinity and PPI/contact rows & Protein-protein, protein-ligand, protein-RNA, protein-DNA, receptor-ligand, antibody-antigen, and mixed complex scalar affinity records. & Use all local reviewed rows; affinity-weight contacts only when affinity fields exist. \\
10 & OpenMathReasoning TIR & Tool-integrated mathematical reasoning and verifier-like traces. & Cap at 1.3M rows. \\
11 & OpenMathReasoning GenSelect & Solution-selection supervision for ranking and diversity-quality tradeoffs. & Cap at 300k rows. \\
12 & DCLM baseline 1B slice & Curated general language and fluency support without pulling a multi-trillion-token corpus. & Lowest-priority active addition. \\
\bottomrule
\end{longtable}}
...
Oracle/verifier & UMA validity/reward bins, temperature-conditioned score bins, optional generated-coordinate force feedback, tool status, repair hints, verifier confidence & External scoring, force-feedback surrogates, and GFlowNet rewards, not copied structure-file energy/force labels. \\
...
\]
Persistent high mass in \(C_t(i,j)\), especially for sequence-distant pairs, is interpreted as a latent contact hypothesis. The motion \(\Delta C_t=C_{t+1}-C_t\) is a folding-like graph-state motion: contacts form, dissolve, or sharpen as the latent state evolves. In the current implementation these maps are source-token maps. They become all-atom and bond-aware when the graphifier inserts all-atom template nodes and covalent \texttt{molecular\_bond} edge tokens. Broad million-row BioSELFIES rows remain compact, but bounded static/contact and structure-dynamics rows include unfolded all-atom template atoms, residue/base/SELFIES source tokens, and bond edge tokens under an 8192-source-token budget. For proteins, indices may therefore denote residues, atom slots, or bond tokens; for SELFIES they may denote atom slots, functional groups, or covalent edge tokens; for RNA and DNA they may denote bases, sugar-phosphate atoms, pairing hypotheses, or bond tokens.

The coordinate and frame decoder is autoregressive. It emits candidate atom, bond, coordinate, and frame records as ordinary output graph tokens. In the implementation, a bounded generated position record has the form
\[
  \texttt{COORD:f}f\texttt{:a}i\texttt{:}u\texttt{:}b,
\]
where \(f\) is a frame index, \(i\) is an atom or residue-associated atom slot, \(u\in\{x,y,z\}\) is the coordinate axis, and \(b\) is a signed coordinate bin such as \(\texttt{neg\_mid}\), \(\texttt{zero}\), or \(\texttt{pos\_near}\). The renderer may map the three axis-bin records for a fixed \((f,i)\) to a representative coordinate or interval. This design keeps generated positions in the same random-order conditional law as text, proof, SELFIES, tool, oracle, atom, and bond records.

The codebase now also contains an optional continuous coordinate head. In the active sequence-first oracle path this head is not trained from copied structure labels. Instead, source-side \(\texttt{UMA\_COORD\_QUERY}\) and all-atom Cartesian query slots are derived from sequence or string records. Protein rows initialize an approximately linear, unfolded heavy-atom template: backbone atoms appear first, side-chain heavy atoms are added by residue template under the configured atom budget, and covalent peptide/side-chain bonds are represented as edge tokens. DNA/RNA rows analogously initialize sugar-phosphate/base atom templates, and SELFIES/SMILES rows read atom and bond symbols from the molecular string parser when available. These slots ask the graph reasoner to propose all-atom Cartesian coordinates and folding-like updates from an initially unfolded symbolic template. The coordinate head is therefore a readout from the same embedding-space graph-of-thought state that supports text, proof, tool, contact, and oracle records; it is not a separate structure predictor that bypasses latent GoT computation.

Let \(q\) be a coordinate-query or coordinate-record slot. The transformer hidden state \(h_q\) is mapped to
\[
  (\mu_q,\ell_q)=H_{\rm coord}(h_q)\in\mathbb R^3\times\mathbb R^3,
\]
where \(\mu_q\) is a normalized mean coordinate and \(\ell_q\) is a clipped log-variance vector. The implemented UMA force-feedback loss uses the mean coordinate \(x_q=s\mu_q\) for generated candidates and keeps supervised coordinate likelihood disabled. For a candidate atom list \(a_1,\ldots,a_n\), proposed positions \(x_t=(x_{t,1},\ldots,x_{t,n})\), and continuous graph temperature \(T\), UMA returns an energy and forces
\[
  E_t = E_{\mathrm{UMA}}(a_{1:n},x_t,T),
  \qquad
  F_t=-\nabla_x E_{\mathrm{UMA}}(a_{1:n},x_t,T).
\]
The oracle is external and is treated as a stop-gradient evaluator. A force-alignment surrogate is
\[
  \mathcal L_{\mathrm{UMA\mbox{-}coord}}
  =
  -\frac1n\sum_{t=0}^{K-1}\sum_{i=1}^n
  \left\langle \operatorname{sg}(F_{t,i}), x_{t,i}\right\rangle
  +\lambda_c\left\|\frac1n\sum_i x_{0,i}\right\|^2
  +\lambda_r\frac{1}{|\mathcal P|}\sum_{i<j}\max(0,r_{\min}-\|x_{0,i}-x_{0,j}\|)^2 .
\]
Here \(\operatorname{sg}\) denotes stop-gradient. Gradient descent on the first term moves \(x_{t,i}\) in the UMA force direction. The rollout
\[
  x_{t+1}=x_t+\eta\,\operatorname{sg}(F_t)
\]
turns reasoning iterations into a short physical-time discretization: the model proposes a geometry, the oracle scores it at the graph temperature, the generated coordinates are advanced by detached forces, and the next score is logged and trained. Low-temperature examples emphasize stabilization and refinement; high-temperature examples preserve broader contact and coordinate support when combined with the contact-entropy and GFlowNet diversity terms. The gradients still flow through \(H_{\rm coord}(h_q)\) into the hidden embeddings, attention/contact maps, and graph-state reasoning layers, so the mechanism maintains embedding-space GoT reasoning rather than replacing it with a coordinate-only model.

For completeness, a future supervised structure phase could use the same head with masked Gaussian likelihood. If \(y_q\in\mathbb R^3\) is an available coordinate target, \(m_q\in\{0,1\}^3\) is the axis mask, and \(s>0\) is a coordinate scale, the masked coordinate loss would be
\[
  \mathcal L_{\rm coord}
  =
  \frac{1}{\sum_{q,u}m_{q,u}}
  \sum_{q,u}
  m_{q,u}
  \frac12
  \left[
    \ell_{q,u}
    +
    \left(\frac{y_{q,u}}{s}-\mu_{q,u}\right)^2\exp(-\ell_{q,u})
  \right].
\]
Only coordinate-bearing graph-token positions have nonzero mask. Thus the symbolic autoregressive decoder still decides which coordinate record exists and what frame, atom slot, and axis it names; the coordinate head supplies a continuous refinement and uncertainty estimate for that already named graph record. In the current UMA path this supervised likelihood has weight zero.

\begin{proposition}[Disabled Gaussian coordinate-supervision reference]
Assume a structure phase supplies samples \((X,Y,M)\), where \(X\) is the input and partial output graph state, \(Y\) is a continuous coordinate target, and \(M\) is an axis mask. Among all measurable coordinate heads with diagonal Gaussian outputs, minimizing the expected masked negative log likelihood above recovers the conditional masked mean and variance:
\[
  \mu^*(X)=\mathbb E[Y/s\mid X,M=1],
  \qquad
  \exp(\ell^*(X))=\operatorname{Var}(Y/s\mid X,M=1)
\]
on axes with positive mask probability.
\end{proposition}
\begin{proof}
For a fixed graph state \(X=x\) and axis \(u\), the masked objective restricts attention to samples with \(M_u=1\). The expected one-dimensional Gaussian negative log likelihood is
\[
  \frac12 \ell + \frac12 \exp(-\ell)\mathbb E[(Z-\mu)^2\mid x,M_u=1],
\]
where \(Z=Y_u/s\). Differentiating with respect to \(\mu\) gives \(\mu=\mathbb E[Z\mid x,M_u=1]\). Substituting this value leaves \(\frac12\ell+\frac12\exp(-\ell)\operatorname{Var}(Z\mid x,M_u=1)\), whose derivative with respect to \(\ell\) is zero when \(\exp(\ell)=\operatorname{Var}(Z\mid x,M_u=1)\). Clipping the implemented log variance bounds this optimum to the configured interval.
\end{proof}

For any rendered generated coordinates \(x_i^f\), define
\[
...
The temperature-conditioned version also uses contact-map entropy as a diversity target. For a graph state with pair mask \(M=\{(i,j):i<j\}\), define the normalized contact entropy
\[
  H(C_t)=-\frac{1}{\log |M|}
  \sum_{(i,j)\in M}p_{ij}\log p_{ij},
  \qquad
  p_{ij}=\frac{C_t(i,j)+\epsilon}{\sum_{(a,b)\in M}(C_t(a,b)+\epsilon)} .
\]
Let \(d_T=(T-300)/100\) after clipping to \([0,1]\). UMA-feedback rows with high \(T\) should not collapse to a single contact hypothesis; they should preserve broader conformational support. Low-\(T\) rows should sharpen toward stabilization and refinement. The optional entropy-coupling loss is therefore
\[
  \mathcal L_{\mathrm{UMA\mbox{-}div}}=
  \left(H(C_t)-d_T\right)^2+
  \lambda_E\left(H(C_t^E)-d_T\right)^2,
\]
applied only to examples carrying explicit UMA coupling, influence, motion, or trajectory records. This term trains both the attention-contact field and the embedding geometry: high-temperature graph states favor \texttt{explore}, \texttt{diversify}, and \texttt{expand} records, while low-temperature graph states favor \texttt{stabilize}, \texttt{refine}, and \texttt{contract}. It is still oracle-conditioned graph-state learning, not supervised MD trajectory fitting.
...
Random-order autoregression is the active implementation path. A policy chooses the next missing record and samples it from the same TokenGT conditional distribution used for every output record. Masked discrete diffusion and discrete flow matching remain theoretical comparison points and possible future ablations for categorical graph completion; they are not used as active losses in the present training code. In the sequence-first UMA run, generated structure positions are represented by autoregressive coordinate graph tokens and by optional coordinate-query readouts trained through UMA force feedback on model-generated candidates. Supervised masked Gaussian coordinate loss remains available as a disabled reference objective and is not part of the default structure-file-free curriculum.
...
When the coordinate readout is enabled, the oracle call can also return forces on the model's own generated coordinates. These forces are not labels from a training set; they are feedback from evaluating the current candidate. The same temperature \(T\) controls both the graph-state policy and the oracle force evaluation. Thus higher \(T\) affects two coupled objects: the embedding/contact trajectory should maintain more conformational alternatives, and the generated coordinate rollout should sample broader motion before selection. Lower \(T\) should concentrate both contact maps and coordinate updates around more stable refinements.
...
def build_temperature_conditioned_trace(input_graph):
    ...
    for T in sample_temperature_schedule():
        ...
        for step in range(max_graph_state_steps):
            ...
            if G_next.has_generated_coordinates():
                forces = UMA_force_feedback(G_next.coordinates, temperature=T)
                G_next = G_next.with_force_guided_coordinate_rollout(forces)
            traces.append((G, action, G_next, T, oracle.reward_bin))
            G = repair_or_continue(G_next, oracle)
    return traces
\end{lstlisting}

This is the role of the high-to-low temperature annealing ladder in the current paper. It creates a curriculum of \emph{continuous conditioning features and oracle-scored graph-state decisions}. It should improve whether the model interprets \(T=397.5\,\mathrm K\) differently from \(T=302.5\,\mathrm K\), and whether its GoT/ToT/GFlowNet policy searches broadly at high temperature and more selectively at low temperature. It is not an instruction to train on actual structure or dynamics files. In the implementation, this distinction is expressed twice: the random-order graph target includes temperature-dependent motion records, and the GFlowNet reward optionally adds a high-\(T\) terminal-state diversity bonus so that hot trajectories are not merely accepted more often but are sampled from a broader set of candidate conformational graph states.
...
For conformational search one may further use
\[
  R_T^{\mathrm{div}}(Y_G)=R_T(Y_G)\left(1+\eta d_T D(Y_G;\mathcal B_T)\right),
\]
where \(d_T=(T-300)/100\), \(\mathcal B_T\) is the current batch or replay set of high-temperature terminal graph states, and \(D\) is a normalized graph-edit or Hamming diversity score over generated conformation/contact records. This term is zero or small at low temperature and positive near \(400\,\mathrm K\). It does not replace the UMA energy/force reward; it tells the sampler that high-temperature oracle queries should cover multiple plausible conformations rather than repeatedly returning one refined low-temperature state.
...
The expanded corpus and split program in Section~\ref{sec:additional-datasets} should be treated as the primary replication specification. The implemented curation path is concrete: source-specific preparation scripts normalize FASTA, TSV, CSV, JSON, JSONL, parquet, SELFIES, SMILES, UniProt feature exports, and affinity tables into graph JSONL; exact raw-row and row-hash deduplication remove duplicate records; entity-aware split keys use protein sequence clusters, molecule scaffolds or InChIKey-like keys, RNA family identifiers, document keys, and row hashes; fast-copy curation streams JSONL directly into split files; resumable temporary split files and a resume-state file avoid restarting multi-hour curation runs; and integrity/policy checks verify that split sizes match summaries, JSONL lines are readable, concatenated lines are repaired, and forbidden structure labels are absent.

The implementation distinguishes strict training data from evaluation or future-phase data. Strict rows may contain symbolic sequences, compact BioSELFIES, SELFIES/SMILES strings, reviewed UniProt annotations, GO/EC/domain/feature/binding-site text, scalar assay or affinity measurements, PPI/contact-prior records, temperature fields, model-proposed coordinate query slots, and online UMA/FairChem feedback. They may not contain supervised PDB/mmCIF/SDF coordinates, structure-token labels, dataset-provided contact maps, conformer libraries, MD trajectories, or dataset-provided energy/force labels. The current completed combined run used $574{,}627$ reviewed UniProt feature rows, $1{,}836{,}729$ biomolecular-complex affinity rows, and the $7{,}328{,}008$-row original public selected graph corpus. The current bio-scale pass adds compact BioSELFIES graphification for UniRef50 protein sequences, PubChem10M SELFIES, UniProt function text, Rfam, RNAcentral 8192, and DNA coding-region rows. Broad rows are capped at 8192 BioSELFIES/source characters so million-row modality coverage stays disk-feasible, while all-atom Cartesian templates, covalent bond edge tokens, contact records, and UMA coordinate/internal-coordinate slots are concentrated in bounded static/contact and structure-dynamics subsets.

Contact-map supervision is also split by provenance. ESM-style attention/contact heads and OMG/gLM2 categorical-Jacobian couplings are treated as sequence-derived contact priors. Affinity-weighted PPI/contact records are created only for complex rows that actually contain affinity fields such as \(K_d\), \(K_i\), IC50, \(k_{\rm on}\), \(k_{\rm off}\), or \(\Delta G\). Therefore not every contact record has measured affinity. The training and evaluation reports must preserve this distinction rather than pooling all contacts under one label family.

The table below is a modality-level summary for the sequence-first multimodal component; the complete implementation uses the entity-aware split registry of Section~\ref{sec:entity-aware-splits}. Resources with actual structures or trajectories are marked evaluation-only unless a later, explicit phase enables them.
...
Complex affinity and PPI/contact priors & Reviewed UniProt features, biomolecular-affinity tables, BindingDB/PDBbind-derived scalar rows when sanitized, OMG/gLM2-style sequence context & PPI/contact records, scalar affinity normalization, interface evidence, categorical-Jacobian or attention-derived contact priors without supervised coordinates. & Refs. \\
Bounded static/contact and dynamics subsets & Compact sequence rows with generated all-atom templates, bond edge tokens, coordinate-query slots, and UMA rewards & All-atom Cartesian action slots, internal-coordinate actions, contact patches, unfolded initial coordinates, and structure-dynamics GFlowNet targets; no copied structure files. & Refs. \\
...
The current implementation separates three mechanisms that are easy to conflate. First, \texttt{tropical} training diagnostics rescale output logits and report entropy, top-1 margin, and confidence; this does not change the attention kernel. Second, \texttt{model.attention\_backend=tropical} activates masked Multi-head Tropical Attention in every TokenGT encoder block. Third, \texttt{model.attention\_backend=hybrid\_flash\_tropical} uses fast softmax attention in every layer and MHTA only on the configured layer set, for example \texttt{hybrid\_tropical\_layers=[-1]} for final-layer MHTA. These backends log internal Hilbert-score statistics, distance means, max-selection confidence, argmax diversity, tropical context scale, FlashAttention/SDPA path use, exact query-chunk size, and the fraction of hybrid layers that actually executed MHTA. Because unchunked MHTA has an intermediate tensor scaling as \([B,H,S,S,d_h]\), full MHTA is an ablation tool for graph-reasoning and algorithmic tasks, while exact query-chunked, layer-sparse hybrid attention is the practical local-training path.
...
5 & UMA temperature conditioning & Train continuous \(T\in[300,400]\,\mathrm K\) oracle-feedback records plus UMA-modulated attention-coupling, token-motion bins, and optional force-feedback coordinate rollouts, using \(400\to300\,\mathrm K\) anchors as a curriculum scaffold. & Model-generated sequence-only candidates and generated coordinates scored by UMA-style oracles and validators. \\
...
for batch in dataloader:
    ...
    loss_oracle = 0.0; loss_uma_coord = 0.0; loss_gfn = 0.0
    ...
    if uma_coordinate_head_enabled:
        loss_uma_coord = uma_force_feedback_loss(model.generated_coordinates,
                                                 pairs.temperature,
                                                 dynamics_steps=K)
    if gflownet_stage:
    ...
    loss = weighted_sum(loss_roa, loss_seq, loss_reason, loss_oracle, loss_uma_coord,
                        loss_gfn, loss_attn, loss_id, loss_top)
...
Masked discrete diffusion & future ablation only & & & \\
Discrete flow matching & future ablation only & & & \\
...
B. Hashemi, K. Pasque, C. Teska, and R. Yoshida. Tropical attention: neural algorithmic reasoning for combinatorial algorithms. \emph{NeurIPS}, 2025; arXiv:2505.17190v2.
