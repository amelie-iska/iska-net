...
The codebase now also contains an optional continuous coordinate head. In the active sequence-first oracle path this head is not trained from copied structure labels. Instead, source-side \(\texttt{UMA\_COORD\_QUERY}\) slots are derived from sequence or string records: for proteins a bounded coarse backbone list \((N,C,C,O)\) is created per residue up to the configured cap, and for SELFIES/SMILES molecules atom symbols are read from the molecular string parser. These slots ask the graph reasoner to propose candidate coordinates. The coordinate head is therefore a readout from the same embedding-space graph-of-thought state that supports text, proof, tool, contact, and oracle records; it is not a separate structure predictor that bypasses latent GoT computation.
