...
printf 'RUN_ID=%s\n' "$RUN_ID"
printf 'PREPARE_FULL_BIOMED_SOURCES=%s PREPARE_UNIPROT=%s PREPARE_AFFINITY=%s CURATE_DATA=%s FAST_CURATE=%s TRAIN_PHASES=%s\n' \
  "$PREPARE_FULL_BIOMED_SOURCES" "$PREPARE_UNIPROT" "$PREPARE_AFFINITY" "$CURATE_DATA" "$FAST_CURATE" "$TRAIN_PHASES"
