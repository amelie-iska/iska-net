...
Keep `CURATE_INDEX_ONLY=1` for all all-atom affinity/contact runs. Curation still deduplicates and splits by entity, but the curated corpus stores JSONL references `{path, offset, sha1}` instead of duplicating the huge all-atom affinity rows. The training dataset loader dereferences those offsets lazily. The static/structure-dynamics subset builder is streaming and bounded: the direct wrapper passes `--source-path-include all_atom_contact`, the builder scans only those all-atom source JSONL files sequentially, keeps only the requested top-k candidates in memory, and writes lightweight reference rows for the 25k static and 2,500 dynamics subsets. Disable this only for small smoke corpora or when fully materialized curated splits are explicitly needed.
