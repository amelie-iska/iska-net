...
def test_fast_curate_resume_adds_missing_temp_newline_before_append(tmp_path: Path):
    rows = [ex.to_dict() for ex in iter_synthetic_examples(3)]
    input_path = tmp_path / "input.jsonl"
    out_dir = tmp_path / "curated"
    out_dir.mkdir()
    lines = [json.dumps(row, ensure_ascii=False, sort_keys=True) for row in rows]
    ...
    for split in ("train", "val", "test"):
        (out_dir / f".{split}.jsonl.tmp").write_text("", encoding="utf-8")
    (out_dir / ".train.jsonl.tmp").write_text(lines[0], encoding="utf-8")
    ...
    text = (out_dir / "train.jsonl").read_text(encoding="utf-8")
    assert summary["kept_rows"] == 3
    lines_out = [line for line in text.splitlines() if line.strip()]
